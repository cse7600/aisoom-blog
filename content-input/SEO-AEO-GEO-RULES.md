...
## 11. 출처 링크 품질 세부 규정 (Phase 9.7.1)

**기준일**: 2026-04-15
**상위 문서**: `CLAUDE.md` → "출처 링크 품질 규정 (Phase 9.7.1)"
**실측 근거**: Phase 9.7.1 사전 감사 결과 15건 중 3건 실제 실패(congress.gov 해외, gbsc.or.kr DNS 소멸, seoulallnet.or.kr 응답 불가). 나머지 12건은 Node fetch UA 차단(false positive)으로 재판정.

### 11.1 도메인 신뢰 Tier (프롬프트 작성 우선순위)

| Tier | 조건 | 사용 우선도 |
|------|------|-------------|
| Tier 1 | `.go.kr` 정부 도메인 (kostat.go.kr, nts.go.kr, police.go.kr, iros.go.kr, data.go.kr 등) | 최우선 — 반드시 1개 이상 |
| Tier 2 | `.or.kr` 실존 공공기관 (semas.or.kr, nps.or.kr, kcomwel.or.kr, kfb.or.kr, kisa.or.kr 등) | 2순위 — 보조 근거 |
| Tier 3 | 대형 제조사·공식 서비스 (hyundai.com, samsung.com, lge.co.kr, millie.co.kr, naver.com 공식 블로그) | 3순위 — 제품 사양·약관 출처용 |
| Tier 4 | 파트너·자사 (corp.apply.kr, keeper.ceo/blog) | 보조 참고만. 단독 사용 금지 |

### 11.2 출처 링크 작성 규칙

- 포스트당 출처 **최소 3개**, 이 중 Tier 1 또는 Tier 2가 **최소 1개** 필수.
- 해외 기관 출처(congress.gov, europa.eu, gdpr.eu 등)는 **원본 필요 시에만** 사용하고 한국 맥락 보강용 Tier 1/2를 별도 추가.
- URL은 상세 페이지(publication/press/notice)를 우선. 루트 도메인(`https://www.nts.go.kr/`)만 걸면 재검증에서 자동 폴백 대상.
- 출처 앵커 텍스트는 "기관명 — 페이지 제목" 형식. 예: `중소벤처기업부 — 스마트상점 기술보급사업 공고`

### 11.3 금지 도메인 (Phase 9.7.1 블랙리스트)

| 도메인 | 이유 | 대체 |
|--------|------|------|
| `gbsc.or.kr` | 경기도 소상공인지원센터 DNS 말소 | `semas.or.kr` (소상공인시장진흥공단) |
| `seoulallnet.or.kr` | 2026-04 시점 응답 불가 | `data.seoul.go.kr` 또는 `semas.or.kr` |

신규 블랙리스트 추가 시 `scripts/lib/source-registry.mjs::DOMAIN_BLACKLIST`에만 등록한다. 프롬프트 본문에 도메인을 하드코딩하면 자동 교체 파이프라인이 감지 못한다.

### 11.4 프롬프트 필수 삽입 블록 (4개 어필리에이트 공통)

각 `content-input/prompts/*.md` 파일에 아래 블록을 `SEO & 콘텐츠 규칙` 하위에 통일 삽입:

```
### 출처 품질 규칙 (2026-04-15 기준, Phase 9.7.1)

- 출처는 3개 이상. 최소 1개는 정부(.go.kr) 또는 공공기관(.or.kr 실존).
- 해외 기관 링크는 필요 시에만, 한국 맥락 Tier 1/2를 반드시 병기.
- URL은 상세 페이지(공고/보도자료/통계 지표) 우선. 루트 도메인 금지.
- 앵커 텍스트는 "기관명 — 페이지 제목" 형식. 예: "국세청 — 법인세 신고안내".
- 금지 도메인: gbsc.or.kr, seoulallnet.or.kr (DNS 소멸 / 응답 불가).
- 자사 파트너 블로그(corp.apply.kr/blog, keeper.ceo/blog)는 참고만. 단독 출처 금지.
- 대체 URL이 없으면 해당 출처 라인을 작성하지 않는다. 빈 링크·죽은 링크보다 없는 편이 낫다.
```

### 11.5 검증 파이프라인과의 연계

작성 후 게시 전에 아래 순서로 실행:
1. `node scripts/verify-sources.mjs --affiliate {이름}` — exit 1이면 교체 필요.
2. `node scripts/repair-sources.mjs --affiliate {이름}` (dry-run).
3. 리포트 검토 → `--apply`로 실제 수정.
4. 게시 후 24시간 이내 `node scripts/check-sources-supabase.mjs --dry --status published,scheduled`로 DB 감사.

Why — Google 2026 March Core Update는 외부 링크의 destination quality를 E-E-A-T Trust 신호로 평가한다. 죽은 링크·무관 페이지는 페이지 전체의 citation 가중치를 2.1배 감쇠시킨다(실측: 300자당 인용 차이).
...
| 2026-04-15 | 출처 품질 세부 규정 (v1.2) | Phase 9.7.1 — 도메인 tier, 블랙리스트, 프롬프트 공통 블록, verify/repair/check-supabase 파이프라인 연계 |
